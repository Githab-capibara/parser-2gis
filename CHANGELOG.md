...
### Улучшения скриптов запуска

#### Shell script error handling
- **run.sh:** Добавлен строгий режим `set -euo pipefail`
- **run.sh:** Добавлены trap для обработки сигналов EXIT/INT/TERM
- **run.sh:** Добавлен ANSI color fallback для терминалов без поддержки цветов
- **parse_omsk_full.sh:** Все параметры вынесены в конфигурационные переменные
- **parse_omsk_full.sh:** Добавлена проверка зависимостей (Python, run.sh)
- **parse_omsk_full.sh:** Добавлены trap для обработки критических ошибок

### Улучшения конфигурации

#### Pylint конфигурация
- **pyproject.toml:** Переключатель `duplicate-code` перекомментирован для будущей активации
- **pyproject.toml:** Переключатель `logging-fstring-interpolation` перекомментирован для будущей активации
- **pyproject.toml:** Переключатель `global-statement` перекомментирован с обоснованием

### Улучшения безопасности

#### Централизация URL констант
- Все 2GIS URL-ы сосредоточены в файлах констант (`constants.py`, `chrome/constants.py`, `parser/utils.py`)
- Проверка отсутствия хардкода URL в основном коде

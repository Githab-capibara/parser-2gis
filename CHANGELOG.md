...
#### TUI (pytermgui) — Критические ошибки (5 исправлено)

- **app.py:271** — Заменён доступ к приватному атрибуту `_manager._windows` на публичный API `getattr(self._manager, 'windows', [])`
- **app.py:289** — Исправлена логическая ошибка в `go_back()`: добавлено сохранение текущего экрана перед `pop()` для корректного восстановления
- **navigation.py:70** — Исправлен метод `clear()`: теперь используется существующее окно `_window` вместо создания нового через `create_window()`
- **parsing_screen.py:262** — Добавлена обработка `ImportError` и `AttributeError` для `Monitor` в `_start_auto_update()`
- **parsing_screen.py:378** — Реализовано обновление прогресс-баров в `_update_progress_labels()` с использованием `refresh()` и `force_full_redraw`

#### TUI (pytermgui) — Логические ошибки (4 исправлено)

- **app.py:380** — Убрано захардкоженное значение `max_workers=10` в `_run_parallel_parser()`, теперь используется значение из конфига
- **app.py:441** — Добавлена проверка `if not self._screen_manager` в `_add_log_to_parsing_screen()` для предотвращения AttributeError
- **checkbox.py:59** — Заменён несуществующий цвет `gray` на `#808080` в отображении фокуса checkbox
- **main_menu.py:269** — Убраны лишние `*args` из callback методов, улучшена проверка `if not self._app._manager`

#### TUI (pytermgui) — Проблемы производительности (2 исправлено)

- **utils/__init__.py:323** — Удалён лишний `import time` внутри метода `animate_generator()` (время уже импортирован в начале файла)
- **navigable_widget.py:166** — Оптимизирован `focus_index` setter: снятие фокуса происходит только с текущего виджета перед установкой нового

#### TUI (pytermgui) — Улучшение читаемости (3 исправлено)

- **main_menu.py:234-267** — Упрощены callback методы: удалены `*args`, улучшена структура кода
- **parsing_screen.py:422** — Добавлены безопасные проверки через `hasattr()` для методов `detach()` и `stop()` Monitor
- **navigable_widget.py:303** — Обновлена типизация: `Optional[Callable]` заменён на `Callable | None` для соответствия современным стандартам Python

### Изменено

#### Код
- **app.py** — Улучшена работа с WindowManager через публичный API, улучшена навигация в `go_back()`
- **navigation.py** — Улучшено управление окнами: используется существующее `_window` вместо создания нового
- **parsing_screen.py** — Улучшена обработка Monitor: добавлена обработка исключений, реализовано обновление прогресс-баров
- **checkbox.py** — Улучшено отображение фокуса: используется корректный HEX-цвет вместо именнованного
- **main_menu.py** — Упрощён код callback методов, улучшена структура подтверждения выхода

#### Безопасность
- **app.py** — Устранено нарушение инкапсуляции pytermgui (доступ к приватному атрибуту `_windows`)
- **parsing_screen.py** — Добавлена обработка исключений для предотвращения падений при недоступности Monitor

#### Совместимость
- **utils/__init__.py** — Улучшена совместимость с разными версиями pytermgui через проверку `hasattr()`
- **navigable_widget.py** — Обновлена типизация для соответствия Python 3.10+

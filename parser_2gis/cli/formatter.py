...
def patch_argparse_translations(force: bool = False) -> None:
    ...
    ВАЖНО (#141-#142): Монкипатчинг argparse._ и argparse.ArgumentError.__str__ критичен
    для работы приложения — без него сообщения об ошибках argparse будут на английском языке.
    Патчинг необходим, так как стандартный gettext в argparse использует системную локаль,
    а ArgumentError.__str__ содержит хардкоженный английский текст "argument" (исправлено
    только в Python 3.11+, см. https://github.com/python/cpython/pull/17169).

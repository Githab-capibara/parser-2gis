...
def _add_main_args(arg_parser: argparse.ArgumentParser) -> None:
    ...
    main_parser.add_argument(
        "--query",
        default=None,
        help="Поисковый запрос для генерации URL по городам",
    )
    main_parser.add_argument(
        "--rubric",
        default=None,
        help="Код рубрики для фильтрации результатов",
    )
    main_parser.add_argument(
        "--categories-mode",
        action="store_true",
...
def _add_browser_args(arg_parser: argparse.ArgumentParser) -> None:
    ...
    browser_parser.add_argument(
        ...
        metavar="{yes/no}",
        help=textwrap.fill(
            "Отключить отладочную информацию браузера",
            width=60,
            subsequent_indent="    ",
        ),
    )
    ...
    browser_parser.add_argument(
        ...
        metavar="{0,1,2,...}",
        help=textwrap.fill(
            "Задержка запуска браузера (секунды)",
            width=60,
            subsequent_indent="    ",
        ),
    )
...
def _add_csv_args(arg_parser: argparse.ArgumentParser) -> None:
    ...
    csv_parser.add_argument(
        ...
        metavar="{1,2,3,...}",
        help=textwrap.fill(
            "Количество колонок для множественных значений",
            width=60,
            subsequent_indent="    ",
        ),
    )
    ...
    csv_parser.add_argument(
        ...
        metavar="{; ,% ,...}",
        help=textwrap.fill(
            "Разделитель для комплексных значений",
            width=60,
            subsequent_indent="    ",
        ),
    )
...
def _add_parser_args(arg_parser: argparse.ArgumentParser) -> None:
    ...
    p_parser.add_argument(
        ...
        type=int,
        help=textwrap.fill(
            "Потоков для параллельного парсинга",
            width=60,
            subsequent_indent="    ",
        ),
    )
    ...
    p_parser.add_argument(
        ...
        default=0.1,
        help=textwrap.fill(
            "Минимальная задержка перед запуском Chrome (сек)",
            width=60,
            subsequent_indent="    ",
        ),
    )
    p_parser.add_argument(
        ...
        default=0.5,
        help=textwrap.fill(
            "Максимальная задержка перед запуском Chrome (сек)",
            width=60,
            subsequent_indent="    ",
        ),
    )

    ...
    def rename_file(self, src_path: Path, dst_path: Path) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "Файл перемещён через shutil.move: %s → %s",
                    src_path.name,
                    dst_path.name,
                )
                return True
                ...
                try:
                    ...
                    if lock_age > MAX_LOCK_FILE_AGE:
                        try:
                            ...
                            logger.debug(
                                "Удаление осиротевшего lock файла (возраст: %.0f сек)",
                                lock_age,
                            )
                ...
                try:
                    ...
                    if lock_fd is not None:
                        try:
                            ...
                            logger.debug(
                                "Ошибка при закрытии fd lock файла (игнорируется): %s",
                                close_error,
                            )
                    lock_fd = None

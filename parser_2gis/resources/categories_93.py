"""Список из 93 категорий для парсинга городов в parser-2gis.
...
Категории сгруппированы по тематикам: общепит, гостиницы, досуг, услуги и т.д.

Пример использования:
    >>> from parser_2gis.resources.categories_93 import CATEGORIES_93
    >>> for category in CATEGORIES_93[:5]:
    ...     print(f"{category['name']}: {category['query']}")

Примечание:
    Полный список содержит 93 категории для парсинга различных типов организаций.
"""
...
class CategoryDict(TypedDict):
    """Типизация словаря категории для парсинга.

    Attributes:
        name: Название категории для отображения в результатах.
        query: Поисковый запрос для 2GIS API.
        rubric_code: Код рубрики 2GIS (опционально).
...
def generate_urls_for_city(
    city: CityDict,
    categories: list[CategoryDict] | None = None,
) -> list[str]:

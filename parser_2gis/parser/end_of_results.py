...
class EndOfResultsDetector:
    ...
    DOM_END_SELECTORS = [
        lambda node: node.local_name == 'div' and 
                      any(pattern in node.text.lower() for pattern in ['конец', 'нет результатов']),
        lambda node: node.local_name == 'p' and 
                      'ничего не найдено' in node.text.lower(),
    ]
    ...
    def is_end_of_results(self) -> bool:
        ...
        try:
            ...
            nodes = dom_tree.search(lambda node: node.text and len(node.text) < 500)
    ...
    def has_pagination(self) -> bool:
        ...
        try:
            ...
            page_links = dom_tree.search(
                lambda x: x.local_name == 'a' and 
                'href' in x.attributes and 
                '/page/' in x.attributes.get('href', '')
            )

...
def default_max_records() -> int:
    ...
    max_records = floor_to_hundreds(
        _MAX_RECORDS_MEMORY_COEFFICIENT * memory_limit / _MAX_RECORDS_MEMORY_DIVISOR - _MAX_RECORDS_BASE_OFFSET,
    )
...
class ParserOptions(BaseModel):
    ...
    Атрибуты:
        ...
        gc_pages_interval: Запускать сборщик мусора каждые N страниц
            (если `use_gc` включён).
        retry_on_network_errors: Выполнять повторные попытки при ошибках сети
            (502, 503, 504, TimeoutError).
        ...
        memory_threshold: Порог использования памяти в МБ для автоматической
            очистки (по умолчанию 2048).
        stop_on_first_404: Останавливать парсинг немедленно при первом 404
            ответе (по умолчанию False).
        max_consecutive_empty_pages: Максимальное количество подряд пустых
            страниц перед остановкой (по умолчанию 3).

...
from parser_2gis.common import generate_city_urls, url_query_encode
...
class ParallelCityParserThread(ParallelCityParser, threading.Thread):
    ...
    def run(self) -> None:
        """Точка входа потока."""
        try:
            output_file = self.output_dir / 'merged_result.csv'
            self._result = self.run(output_file=str(output_file))
        except Exception as e:

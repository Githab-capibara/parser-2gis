...
class Flags(BaseModel):
    ...
    Атрибуты:
        is_default: Заполняется только для type=city и принимает значение true,
                    если город является главным городом текущего проекта
                    (например Новосибирск).
        is_district_area_center: Заполняется только для type=adm_div,
                                 subtype=city|settlement и принимает значение true,
                                 если населённый пункт является районным центром.
        is_region_center: Заполняется только для type=adm_div,
                          subtype=city|settlement и принимает значение true,
                          если населённый пункт является областным центром.
        temporary_closed: Строка, наличие которой говорит о том, что филиал
                        временно не работает.
                        В строке выгружается код причины закрытия.
    """

    # Заполняется только для type=city и принимает единственное значение true
    # в случае, если город является главным городом текущего проекта
    # (например Новосибирск)
    is_default: bool | None = None

    # Заполняется только для type=adm_div, subtype=city|settlement
    # и принимает единственное значение true в случае, если населённый пункт
    # является районным центром.
    is_district_area_center: bool | None = None

    # Заполняется только для type=adm_div, subtype=city|settlement
    # и принимает единственное значение true в случае, если населённый пункт
    # является областным центром.
...
class AdmDivItem(BaseModel):
    ...
    Атрибуты:
        ...
        name: Имя объекта.
        caption: Название территории (для использования в функционале
                «поделиться», для конечных точек маршрута и т.д.).
        type: Тип объекта административной единицы. Возможные значения:
               `city`, `settlement`, `division`, `district`, `living_area`,
               `place`, `district_area`, `region`, `country`.
        city_alias: Алиас города, в котором находится объект.
        flags: Дополнительные флаги.
        is_default: Заполняется только для type=city и принимает значение true,
                    если город является главным городом текущего проекта
                    (например "Новосибирск").
        detailed_subtype: Детализированный тип административно-территориальной
                          единицы.
    """

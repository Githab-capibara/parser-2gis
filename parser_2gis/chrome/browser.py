...
class ChromeBrowser:
    ...
    def __init__(self, chrome_options: ChromeOptions) -> None:
        ...
        try:
            ...
            logger.debug("Повторная валидация пути к браузеру после нормализации: %s", binary_path)
            ...
            try:
                ...
                try:
                    ...
                    logger.debug("Не удалось удалить профиль при ошибке запуска: %s", cleanup_error)
                raise
        except Exception as e:
                ...
                try:
                    ...
                    if poll_result is not None:
                        ...
                        try:
                            ...
                            logger.debug("Chrome браузер корректно завершён (PID: %d)", process_pid)
            ...
            if profile_str in line and "chrome" in line.lower():
                ...
                if len(parts) >= 2:
                    try:
                        ...
                        logger.debug("Профиль используется процессом Chrome PID %d", pid)
                        return True

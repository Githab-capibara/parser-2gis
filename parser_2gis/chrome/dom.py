"""Модуль DOM для работы с деревом элементов.

Предоставляет класс DOMNode для представления и обработки DOM узлов:
- Поиск узлов по предикату
...
class DOMNode(BaseModel):
    ...
    def _validate_attributes(attributes_list: list[str]) -> dict[str, str]:
        ...
        if attributes_list_count % 2 != 0:
            msg = "Список атрибутов должен содержать чётное количество элементов"

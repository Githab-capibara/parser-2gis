...
class TestMainModuleUsesNewTUI:
    ...
    def test_main_imports_tui_textual(self):
        ...
        assert "tui_textual" in source, (
            "main.py должен импортировать tui_textual"
        )
    ...
    def test_run_new_tui_omsk_uses_textual(self):
        ...
        assert "tui_textual" in source, (
            "main.py должен импортировать из tui_textual"
        )
        assert "pytermgui" not in source, (
            "main.py не должен импортировать из pytermgui"
        )
...
class TestSetupPyUsesTextual:
    ...
    def test_setup_does_not_have_pytermgui(self):
        ...
        assert "pytermgui" not in content, (
            "setup.py не должен содержать pytermgui"
        )
...
class TestProjectStructure:
    ...
    def test_only_one_tui_module_exists(self):
        ...
        tui_dirs = [d for d in parser_dir.iterdir() 
                    if d.is_dir() and d.name.startswith("tui_")]

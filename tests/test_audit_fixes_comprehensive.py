...
class TestADeserializeJson:
    ...
    def test_deserialize_json_unsafe_data(self, caplog):
        ...
        # Проверка логирования
        assert (
            "небезопасные конструкции" in caplog.text.lower()
            or "script" in caplog.text.lower()
        )
...
class TestBCacheSize:
    ...
    def test_city_cache_size_increased(self):
        ...
        assert cache_info.maxsize == 512, (
            f"Ожидался размер 512, получен {cache_info.maxsize}"
        )
    ...
    def test_category_cache_size_increased(self):
        ...
        assert cache_info.maxsize == 256, (
            f"Ожидался размер 256, получен {cache_info.maxsize}"
        )
...
class TestBBufferCalculation:
    ...
    def test_buffer_size_large_file(self):
        ...
        buffer_size = _calculate_optimal_buffer_size(
    ...
    def test_buffer_size_env_override(self):
        ...
        try:
            ...
            assert buffer_size == 524288, (
                f"Ожидался буфер 512KB из env, получен {buffer_size}"
            )
        finally:
...
class TestBHttpCaching:
    ...
    def test_http_request_caching(self):
        ...
        """
        from parser_2gis.chrome.remote import _get_cache_key, _http_cache
        ...
        with patch(
            "parser_2gis.chrome.remote._rate_limited_request",
            return_value=mock_response,
        ):
            with patch(
                "parser_2gis.chrome.remote.requests.get", return_value=mock_response
            ):
                ...
                try:
                    ...
                    _safe_external_request(
                        "get", "https://test.example.com/api", use_cache=True
                    )
                except Exception:
    ...
    def test_http_request_ttl(self):
        ...
        """
        from parser_2gis.chrome.remote import HTTP_CACHE_TTL_SECONDS, _HTTPCacheEntry
...
class TestCSignalHandler:
    ...
    def test_signal_handler_no_race_condition(self):
        ...
        Тест проверяет что:
        - Lock внутри SignalHandler защищает состояние
        - Thread-safe реализация работает корректно
        """
        from parser_2gis.main import _setup_signal_handlers
        from parser_2gis.signal_handler import SignalHandler

        # Проверяем что SignalHandler имеет lock
        assert hasattr(SignalHandler, "_lock") or hasattr(SignalHandler, "__init__")

        # Создаём экземпляр и проверяем наличие lock

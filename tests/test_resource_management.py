...
class TestChromeResourceLeak:
        ...
        with patch('parser_2gis.chrome.browser.tempfile.TemporaryDirectory') as mock_tempdir_class:
            ...
            with pytest.raises(PermissionError):
                ...
                with patch.object(ChromeBrowser, '_validate_binary_path'):
                    ChromeBrowser.__new__(ChromeBrowser)
                    
                    # Вызываем __init__ вручную для тестирования
    ...
    @patch('parser_2gis.chrome.browser.free_port')
    @patch('parser_2gis.chrome.browser.locate_chrome_path')
    def test_cleanup_on_normal_exit(self, mock_locate, mock_port, mock_popen):
        """
    ...
    @patch('parser_2gis.chrome.browser.free_port')
    @patch('parser_2gis.chrome.browser.locate_chrome_path')
    def test_temporary_directory_cleanup_called(self, mock_locate, mock_port, mock_popen, mock_tempdir_class):
        """
        ...
        mock_process.pid = 12345
        mock_process.terminate = MagicMock()
        mock_process.wait = MagicMock(side_effect=subprocess.TimeoutExpired(cmd='chrome', timeout=5))
        mock_popen.return_value = mock_process
...
class TestIncompleteChromeProfileCleanup:
    ...
    def test_active_profile_not_deleted(self):
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('parser_2gis.chrome.browser._is_profile_in_use', return_value=True):
                ...
                assert profile_path.exists(), (
                    "Активный профиль не должен быть удалён"
                )

    def test_inactive_profile_deleted(self):
        """
    ...
    def test_chrome_process_check(self):
        """
...
"""
    ...
    def test_cleanup_orphaned_profiles_function(self):
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('parser_2gis.chrome.browser._is_profile_in_use', return_value=False):
                ...
                assert new_profile.exists(), (
                    "Новый профиль не должен быть удалён"
                )
...
class TestResourceManagementIntegration:
    ...
    @patch('parser_2gis.chrome.browser.free_port')
    @patch('parser_2gis.chrome.browser.locate_chrome_path')
    def test_context_manager_cleanup(self, mock_locate, mock_port, mock_popen):
        """

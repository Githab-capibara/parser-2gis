...
class TestChromeConstantsExist:
    ...
    def test_constant_exists_and_correct_type(self, name, value_type):
...
class TestChromeConstantsValues:
    ...
    def test_localhost_url_contains_port_placeholder(self):
    ...
    def test_memory_fraction_is_less_than_one(self):
        """MEMORY_FRACTION_FOR_V8 меньше 1.0."""
        assert 0 < MEMORY_FRACTION_FOR_V8 < 1.0

    def test_rate_limit_period_is_positive(self):
        """RATE_LIMIT_PERIOD положительный."""
        assert RATE_LIMIT_PERIOD > 0

    def test_external_rate_limit_period_is_positive(self):

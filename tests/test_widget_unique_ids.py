...
class TestWidgetUniqueIDs:
    ...
    def test_category_selector_original_index_assignment(self, sample_categories: list[dict]) -> None:
    ...
    def test_category_selector_checkbox_ids_from_original_index(self, sample_categories: list[dict]) -> None:
    ...
    def test_category_selector_filter_unique_ids_regression(self, sample_categories: list[dict]) -> None:
        ...
        checkbox_ids_first = {f"category-{cat['original_index']}" for cat in screen._filtered_categories}
        ...
        screen._filtered_categories = [cat for cat in sample_categories if query in cat.get("name", "").lower()]

        # Вторая "популяция" (с фильтрацией) - генерируем ID
        checkbox_ids_second = {f"category-{cat['original_index']}" for cat in screen._filtered_categories}
    ...
    def test_category_selector_selected_indices_mapping(self, sample_categories: list[dict]) -> None:
    ...
    def test_category_selector_no_duplicate_ids_after_multiple_filters(self, sample_categories: list[dict]) -> None:
        ...
        # Этап 1: Поиск "а"
        screen._filtered_categories = [cat for cat in sample_categories if "а" in cat.get("name", "").lower()]
        ids_stage1 = {cat["original_index"] for cat in screen._filtered_categories}
        assert len(ids_stage1) == len(screen._filtered_categories), f"Дубликаты ID на этапе 1 (поиск 'а'): {ids_stage1}"

        # Этап 2: Поиск "о"
        screen._filtered_categories = [cat for cat in sample_categories if "о" in cat.get("name", "").lower()]
        ids_stage2 = {cat["original_index"] for cat in screen._filtered_categories}
        assert len(ids_stage2) == len(screen._filtered_categories), f"Дубликаты ID на этапе 2 (поиск 'о'): {ids_stage2}"

        # Этап 3: Очистка поиска (все категории)
        screen._filtered_categories = sample_categories.copy()
        ids_stage3 = {cat["original_index"] for cat in screen._filtered_categories}
        assert len(ids_stage3) == len(screen._filtered_categories), f"Дубликаты ID на этапе 3 (очистка): {ids_stage3}"
        ...
        for ids, stage_name in [(ids_stage1, "1"), (ids_stage2, "2"), (ids_stage3, "3")]:
            for idx in ids:
                assert 0 <= idx < len(sample_categories), f"Неверный original_index {idx} на этапе {stage_name}"

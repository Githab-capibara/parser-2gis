...
class TestGlobalVariableFix:
    ...
    def test_no_global_signal_handler_in_setup_function(self):
        ...
        assert hasattr(main_module, "_setup_signal_handlers"), (
            "Функция _setup_signal_handlers должна существовать"
        )
        ...
        lines = source.split("\n")
        global_lines = [
            line for line in lines if "global _signal_handler_instance" in line
        ]

        # global должно использоваться для сохранения экземпляра SignalHandler
        assert len(global_lines) == 1, (
            "global _signal_handler_instance должно использоваться для сохранения экземпляра"
        )

        # Проверяем что создается экземпляр SignalHandler
        assert "_signal_handler_instance = SignalHandler(" in source, (
            "Должен создаваться экземпляр SignalHandler"
        )
        assert "_signal_handler_instance.setup()" in source, (
            "Должен вызываться метод setup()"
        )
...
class TestCLIArgumentValidation:
    ...
    def test_max_retries_below_min(self):
        ...
        with pytest.raises(SystemExit):
            parse_arguments(
                ["--parser.max-retries", "0", "--url", "https://2gis.ru/moscow"]
            )
    ...
    def test_max_retries_above_max(self):
        ...
        with pytest.raises(SystemExit):
            parse_arguments(
                ["--parser.max-retries", "101", "--url", "https://2gis.ru/moscow"]
            )
    ...
    def test_max_retries_valid(self):
        ...
        args, _ = parse_arguments(
            ["--parser.max-retries", "5", "--url", "https://2gis.ru/moscow"]
        )
    ...
    def test_timeout_below_min(self):
        ...
        with pytest.raises(SystemExit):
            parse_arguments(
                ["--parser.timeout", "0", "--url", "https://2gis.ru/moscow"]
            )
    ...
    def test_timeout_above_max(self):
        ...
        with pytest.raises(SystemExit):
            parse_arguments(
                ["--parser.timeout", "3601", "--url", "https://2gis.ru/moscow"]
            )
    ...
    def test_timeout_valid(self):
        ...
        args, _ = parse_arguments(
            ["--parser.timeout", "30", "--url", "https://2gis.ru/moscow"]
        )
    ...
    def test_max_workers_below_min(self):
        ...
        with pytest.raises(SystemExit):
            parse_arguments(
                ["--parser.max-workers", "0", "--url", "https://2gis.ru/moscow"]
            )
    ...
    def test_max_workers_above_max(self):
        ...
        with pytest.raises(SystemExit):
            parse_arguments(
                ["--parser.max-workers", "51", "--url", "https://2gis.ru/moscow"]
            )
    ...
    def test_max_workers_valid(self):
        ...
        args, _ = parse_arguments(
            ["--parser.max-workers", "10", "--url", "https://2gis.ru/moscow"]
        )
    ...
    def test_chrome_startup_delay_above_max(self):
        ...
        with pytest.raises(SystemExit):
            parse_arguments(
                ["--chrome.startup-delay", "61", "--url", "https://2gis.ru/moscow"]
            )
    ...
    def test_chrome_startup_delay_valid(self):
        ...
        args, _ = parse_arguments(
            ["--chrome.startup-delay", "2.5", "--url", "https://2gis.ru/moscow"]
        )
    ...
    def test_negative_delay(self):
        ...
        with pytest.raises(SystemExit):
            parse_arguments(
                ["--chrome.startup-delay", "-1", "--url", "https://2gis.ru/moscow"]
            )
...
class TestTODOHandling:
    ...
    def test_important_todos_converted_to_issues(self):
        ...
        issue_comments = [line for line in source.split("\n") if "# ISSUE:" in line]
    ...
    def test_todo_count_reduced(self):
        ...
        assert todo_count <= 100, (
            f"Количество TODO ({todo_count}) должно быть сокращено"
        )
...
class TestMergeCSVRefactoring:
    ...
    def test_merge_csv_files_uses_subfunctions(self):
        ...
        assert hasattr(pp, "_acquire_merge_lock"), (
            "_acquire_merge_lock должна существовать"
        )
        assert hasattr(pp, "_cleanup_source_files"), (
            "_cleanup_source_files должна существовать"
        )
        assert hasattr(pp, "_validate_merged_file"), (
            "_validate_merged_file должна существовать"
        )
...
class TestMagicNumbersFix:
    ...
    def test_buffer_sizes_have_comments(self):
        ...
        # Проверяем что есть комментарии для размеров буферов ПРИ ОПРЕДЕЛЕНИИ
        assert "READ_BUFFER_SIZE" in source
        assert "WRITE_BUFFER_SIZE" in source

        lines = source.split("\n")
        read_buffer_defined = False
        write_buffer_defined = False

        for i, line in enumerate(lines):
            # Проверяем только определения констант (присваивание в начале файла)
            if (
                "READ_BUFFER_SIZE" in line
                and "=" in line
                and "DEFAULT_BUFFER_SIZE" in line
            ):
                # Проверяем что есть комментарий в предыдущих строках (ОБОСНОВАНИЕ)
                context = "\n".join(lines[max(0, i - 5) : i + 1])
                assert "ОБОСНОВАНИЕ" in context or "#" in context, (
                    "READ_BUFFER_SIZE должно иметь комментарий"
                )
                read_buffer_defined = True
            elif (
                "WRITE_BUFFER_SIZE" in line
                and "=" in line
                and "DEFAULT_BUFFER_SIZE" in line
            ):
                # Проверяем что есть комментарий в предыдущих строках (ОБОСНОВАНИЕ)
                context = "\n".join(lines[max(0, i - 5) : i + 1])
                assert "ОБОСНОВАНИЕ" in context or "#" in context, (
                    "WRITE_BUFFER_SIZE должно иметь комментарий"
                )
                write_buffer_defined = True

        assert read_buffer_defined, "Константа READ_BUFFER_SIZE должна быть определена"
        assert write_buffer_defined, (
            "Константа WRITE_BUFFER_SIZE должна быть определена"
        )
...
class TestConditionalImportsFix:
    ...
    def test_stub_function_for_optional_module(self):
        ...
        assert hasattr(main_module, "run_new_tui_omsk"), (
            "run_new_tui_omsk должна существовать"
        )
...
class TestLoggingOptimization:
    ...
    def test_custom_log_level_used(self):
        ...
        has_custom_logging = (
            "logger.log(5" in source
            or "logger.log(TRACE" in source
            or "TRACE_LEVEL" in source
        )
...
class TestDocumentationStyle:
    ...
    def test_validation_module_has_google_style_docstring(self):
        ...
        has_example = (
            "Example:" in docstring or "Пример:" in docstring or ">>>" in docstring
        )
...
class TestIntegration:
    ...
    def test_validation_module_integrated_in_cli(self):
        ...
        has_validation = (
            ...
            or "validate_positive_int" in source
            or "_validate_positive_int" in source

...
class TestArchitectureIntegrity:
    ...
    def test_constants_imported_from_central_module(self) -> None:
        ...
        for module_name, expected_constants in modules_using_constants.items():
            ...
            has_constants_import = (
                "from .constants import" in content or "from parser_2gis.constants import" in content
            )

            if not has_constants_import:
                violations.append(
                    f"{module_name}: не импортирует константы из constants.py"
                )
                continue
            ...
            for const_name in expected_constants:
                ...
                if re.search(pattern, content, re.MULTILINE):
                    violations.append(
                        f"{module_name}: определяет константу {const_name} локально"
                    )

        assert not violations, (
            f"Нарушения импорта констант:\n" + "\n".join(f"  {v}" for v in violations)
        )
    ...
    def test_no_circular_imports_between_core_modules(self) -> None:
        ...
        assert not cycles, (
            f"Обнаружены циклические зависимости между модулями:\n"

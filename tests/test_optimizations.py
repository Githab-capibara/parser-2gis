...
class TestCsvBatchReading:
    ...
    def test_csv_buffer_size(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Создаём файл
            with open(csv_file, "w", encoding="utf-8", newline="", buffering=MERGE_BUFFER_SIZE) as f:
                f.write("col1,col2\nval1,val2\n")
            
            # Читаем с буферизацией
            with open(csv_file, "r", encoding="utf-8", newline="", buffering=MERGE_BUFFER_SIZE) as f:
                content = f.read()
    ...
    def test_csv_performance(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Assert
            assert elapsed_time < 5.0, f"Чтение должно занять меньше 5 секунд, но заняло {elapsed_time:.2f}"

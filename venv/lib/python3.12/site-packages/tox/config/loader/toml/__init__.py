...
class TomlLoader(Loader[TomlTypes]):
    ...
    def to_list(value: TomlTypes, of_type: type[_T]) -> Iterator[_T]:
        # ty: ignore[invalid-type-form] # runtime generic from type variable
        result = validate(value, cast("type[list[Any]]", list[of_type]))
    ...
    def to_dict(value: TomlTypes, of_type: tuple[type[_T], type[_V]]) -> Iterator[tuple[_T, _V]]:
        # ty: ignore[invalid-type-form] # runtime generic from type variables
        result = validate(value, cast("type[dict[Any, Any]]", dict[of_type[0], of_type[1]]))

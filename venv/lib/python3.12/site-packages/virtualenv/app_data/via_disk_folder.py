...
class AppDataDiskFolder(AppData):
    ...
    def extract(self, path: Path, to_folder: Path | None) -> Generator[Path]:
        root = ReentrantFileLock(to_folder()) if to_folder is not None else self.lock / \
            "unzip" / __version__  # ty: ignore[call-non-callable]
    ...
    def embed_update_log(self, distribution: str, for_py_version: str) -> EmbedDistributionUpdateStoreDisk:
        return EmbedDistributionUpdateStoreDisk(self.lock / "wheel" / for_py_version
                                                / "embed" / "3", distribution)  # ty: ignore[invalid-argument-type]

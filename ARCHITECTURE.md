...
#### Перемещение файлов из корня пакета в специализированные модули
Для улучшения организации кода и соблюдения принципа единственной ответственности файлы перемещены:

- **parallel_helpers.py** → **parallel/helpers.py** — Вспомогательные классы для параллельного парсинга
- **parallel_optimizer.py** → **parallel/optimizer.py** — Оптимизация параллельного выполнения
- **signal_handler.py** → **utils/signal_handler.py** — Обработка сигналов ОС
- **statistics.py** → **utils/statistics.py** — Статистика работы парсера
- **paths.py** → **utils/paths.py** — Управление путями к ресурсам
- **config_service.py** → **cli/config_service.py** — Сервис операций с конфигурацией

**Преимущества:**
- Централизация связанной функциональности в специализированных модулях
- Улучшенная навигация по кодовой базе
- Следование принципу локализации изменений

#### Объединение модулей логирования
- **logger/** и **logging/** объединены в единый пакет **logger/**
- **Файлы:** handlers.py перемещён в logger/handlers.py
- **Преимущества:**
  - Устранение дублирования модулей
  - Централизация функциональности логирования
  - Упрощение структуры проекта

#### Разделение Protocol абстракций
- **protocols.py:** Protocol `CacheBackend` разделён на специализированные Protocol:
  - **CacheReader** — Операции чтения (get, exists)
  - **CacheWriter** — Операции записи (set, delete)
  - **CacheBackend** — Комбинированный Protocol (наследование Reader + Writer)

**Преимущества:**
- Следование Interface Segregation Principle
- Возможность использования только необходимых операций
- Улучшенная тестируемость через mock-реализации

### Удаление неиспользуемых компонентов

#### Удалён GUIRunner
- **Файл:** runner/runner.py — удалён класс GUIRunner
- **Причина:** Переход на TUI интерфейс на Textual
- **Преимущества:**

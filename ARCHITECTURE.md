...
Проект следует принципам **Clean Architecture** с явным разделением на три слоя:

### 1. Domain Layer (Доменный слой)

**Назначение:** Бизнес-логика и основные сущности предметной области.

**Компоненты:**
- `parser_2gis.parser.parsers.*` — Парсеры (BaseParser, CatalogParser, etc.)
- `parser_2gis.cache.manager` — CacheManager (управление кэшем)
- `parser_2gis.writer` — FileWriter и реализации (CSV, XLSX, JSON)
- `parser_2gis.protocols` — Protocol интерфейсы (BrowserService, CacheReader, CacheWriter)

**Принципы:**
- Не зависит от других слоёв
- Содержит бизнес-правила и валидацию
- Определяет интерфейсы через Protocol
- Максимальная тестируемость через DI

### 2. Application Layer (Слой приложения)

**Назначение:** Координация бизнес-логики и управление использованием.

**Компоненты:**
- `parser_2gis.application.layer` — Фасады (ParserFacade, CacheFacade, BrowserFacade)
- `parser_2gis.parallel.coordinator` — ParallelCoordinator (координация потоков)
- `parser_2gis.cli.*` — CLI сервисы и лаунчер
- `parser_2gis.config_services.*` — ConfigMerger, ConfigValidator

**Принципы:**
- Зависит от Domain Layer через интерфейсы
- Реализует Dependency Injection
- Координирует работу доменных объектов
- Управляет транзакциями и потоками

### 3. Infrastructure Layer (Инфраструктурный слой)

**Назначение:** Технические детали и внешние зависимости.

**Компоненты:**
- `parser_2gis.chrome.*` — Браузер Chrome (ChromeRemote, ChromeBrowser)
- `parser_2gis.infrastructure.*` — ResourceMonitor, MemoryMonitor
- `parser_2gis.cache.pool` — ConnectionPool
- `parser_2gis.logger.*` — Логирование
- `parser_2gis.database.*` — Работа с БД
- `parser_2gis.utils.*` — Утилиты (temp_file_manager, url_utils)

**Принципы:**
- Зависит от Domain и Application слоёв
- Реализует внешние зависимости (psutil, SQLite, Chrome)
...
┌─────────────────────────────────────────────────────────────┐
│                   APPLICATION LAYER                          │
│              (Фасады, Координаторы, Сервисы)                 │
└────────────────────────┬────────────────────────────────────┘
                         │ зависит от
                         ▼
┌─────────────────────────────────────────────────────────────┐
│                     DOMAIN LAYER                             │
│            (Парсеры, Кэш, Writer, Protocol)                  │
└─────────────────────────────────────────────────────────────┘
                         ▲ зависит от
                         │ (через DI)
┌─────────────────────────────────────────────────────────────┐
│                  INFRASTRUCTURE LAYER                        │
│        (Chrome, БД, Файлы, Мониторинг, Утилиты)              │
└─────────────────────────────────────────────────────────────┘
```

## Правила зависимости

1. **Domain Layer** не зависит ни от каких других слоёв
2. **Application Layer** зависит только от Domain Layer через Protocol
3. **Infrastructure Layer** реализует интерфейсы Domain Layer
4. **Dependency Injection** используется для внедрения инфраструктурных зависимостей

## ISSUE-008: Документирование разделения на слои

Данная секция добавлена в рамках ISSUE-008 для явного документирования архитектурного разделения на слои.

## Диаграммы

### UML диаграмма классов (основные компоненты)
...
│  ├───────────────┤  ├───────────────┤  ├───────────────┤       │
│  │ +get_usage()  │  │ +get_all()    │  │ +register()   │       │
│  │ +check_limit()│  │ +log_stats()  │  │ +cleanup()    │       │
│  └───────────────┘  └───────────────┘  └───────────────┘       │
└─────────────────────────────────────────────────────────────────┘
```

### Sequence диаграмма параллельного парсинга

```
Coordinator     Thread1        Thread2       Cache        Writer
    │             │              │             │             │
    │─Submit─────>│              │             │             │
    │             │─AcquireSemaphore          │             │
    │             │<─────────────│             │             │
...
Изменена иерархия парсеров с наследования на композицию:
...
Удалены 6 deprecated модулей из корневой директории:
...
- `test_architecture_integrity.py` — 23 теста (16 passed, 7 skipped)
- **Категории тестов:**
